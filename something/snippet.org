...
空基类 空基类优化
空类 没有非静态数据成员 没有虚指针（虚函数表，虚继承的虚基类） sizeof = 1
空类作为基类将被优化 sizeof = 0
...
RAII（Resource Acquisition Is Initialization，资源获取即初始化，是使用一个对象，在其构造时获取资源，
在对象生命期控制对资源的访问使之始终保持有效，最后在对象析构的时候释放资源。）
** auto_ptr
auto_ptr 对管理的对象指针是独占的，在拷贝构造和赋值构造非等价，过程中会发生所有权的转移。
auto_ptr 不能用于标准库容器中。

** scoped_ptr
顾名思义，这个智能指针类型只在它定义的作用域有效，不能传递到域外。
因为拷贝构造和赋值构造函数都私有，所以不能作为函数参数，不能作为返回值。
拥有它指向资源的所有权，它管理的指针不能被共享，因此两个实例之间的比较也没有意义。

** shared_ptr
该类型智能指针管理的对象指针可以在多个实例间共享，通过引用计数来记录拥有该对象的实例的数目。
可以跨越模块、跨越线程边界传递。
拷贝构造和赋值构造是等价的，也可以被比较，因此可以用在标准库容器中。
侵入型计数
非侵入型计数
强引用

** weak_ptr
这个智能指针是为了解决循环引用（可能导致内存泄漏）的问题。
...
- 隧道模式 tunnel mode
  用户的整个IP数据包被用来计算 AH 或 ESP 头， AH 或 ESP 头以及 ESP 加密的
  用户数据被封装在一个新的 IP 数据包中。通常，隧道模式应用在两个安全网关之间的通讯。
  真正的 IP 地址隐藏，可进行主机或者拓扑隐藏。
  - 优点
  - 缺点
- 传输模式 transport mode
  只是传输层（ TCP, UDP, ICMP 等）数据被用来计算 AH 或 ESP 头， AH 或 ESP 头
  以及 ESP 加密的用户数据被放置在原 IP 包头后面。通常，传输模式应用在两台主机之间的
  通讯，或一台主机和一个安全网关之间的通讯
  - 优点
  - 缺点
...
#+END_SRC
** 无锁队列 Lock Free
CAS (Compare & Set or Compare & Swap)
...
** mplayer
#+BEGIN_SRC sh
  $ man mplayer # or `man mplayer2'
#+END_SRC
#+BEGIN_SRC sh
  $ mplayer -rawvideo format=help
#+END_SRC
#+BEGIN_SRC sh
  $ mplayer /path/to/video_file.raw -demuxer rawvideo -rawvideo fps=30:w=640:h=480:format=rgba
#+END_SRC
#+BEGIN_SRC sh
  $ mplayer /path/to/audio_file.raw -demuxer rawaudio -rawaudio samplesize=2:channels=1:rate=8000
#+END_SRC
...
#+BEGIN_SRC sh
  $ ffplay -ar 16000 -ac 1 /path/to/audio_file.raw
#+END_SRC
...
#+END_SRC
#+BEGIN_SRC sh
  $ play -t raw -r 16k -e signed -b 16 -c 1 /path/to/audio_file.raw
#+END_SRC
-r = sampling rate
...
IEEE 754 标准中，浮点数表示 (-1)^S*2^E*M 。
S 是符号位， E 是指数位， M 是有效数字，如同十进制的科学计数法。
该标准规定了 S E M 所占的二进制位数，比如 float 类型，该标准规定 S 占 1 位， E 占 11 位， M 占 23 位。
** FPU 浮点运算单元
80 位， x64 上是 128 位。
** 尽量使用 double 型
因为 float 类型进入 FPU 时要扩展成 80 位，会丢失精度。所以下面代码的结果是：
#+BEGIN_SRC c
  /* float_fpu.c */
  #include <stdio.h>
  #include <float.h>
  ...
  int main()
  {
      ...
      (fabs(f) - 1.0f < FLT_EPSILON) ? printf(" equal \n") : printf("not equal\n");
...
#+END_SRC
#+BEGIN_SRC sh
  $ gcc -o float_fpu float_fpu.c
  $ ./float_fpu
  not equal
#+END_SRC
** DBL_EPSILON FLT_EPSILON
EPSILON 是最小误差。是 EPSILON + X 不等于 X 的最小的正数。使用这两个宏来进行浮点数比较操作。
...
#+BEGIN_SRC c
  ...
  if (fabs(b - 0.5) < DBL_EPSILON) // 正确
      x++;
#+END_SRC

另外，我们可以根据自己需要定义这两个宏。
比如在牵涉到货币的运算中，只需要小数点后面两位的精度就可以了，所以，我们可以把这两个宏定义为 0.001 。
